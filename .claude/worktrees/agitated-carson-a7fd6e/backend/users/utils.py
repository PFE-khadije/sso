...
def decrypt_secret(stored: str) -> str:
    ...
    Handles a migration period where old rows may still hold plaintext secrets:
    if decryption fails we return the value as-is so existing TOTP codes keep
    working until an operator re-enrols or runs a data migration.
    """
    try:
        return decrypt_value(stored)
    except Exception:
        # Value was stored before encryption was introduced — treat as plaintext
        return stored